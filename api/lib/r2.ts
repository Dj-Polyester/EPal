...
export async function uploadBase64Image(base64: string, key: string): Promise<string> {
 ...
 */
export function getPublicUrl(url: string | null): string | null {
  if (!url || !r2PublicDomain) return url;

  // If already using public domain, return as-is
  if (url.startsWith(r2PublicDomain)) return url;
